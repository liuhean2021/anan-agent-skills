...
- **Secret 扫描（每次必跑，不受 Diff Scope 影响）**：

  ```bash
  # gitleaks v8.19+：扫描暂存区（与 pre-commit hook 行为一致）
  gitleaks git --pre-commit --staged --redact --verbose
  # 审查范围含未暂存改动时，追加扫描工作区 diff
  gitleaks git --pre-commit --redact --verbose
  # 旧版本 gitleaks：将 `git --pre-commit` 替换为 `protect`，其余参数不变
  ```

  - 按审查范围选择命令：`--staged` 只扫暂存区，不带 `--staged` 只扫未暂存 diff；两处都有改动时两条都跑。
  - 以输出中的 `scanned ~N bytes` 判断是否真正扫到内容：`N = 0` 而审查范围内有改动时，**不得视为通过**，应换用另一条命令重扫。
  - 已配置 gitleaks pre-commit hook 的项目，仍须在审查时主动执行一次，而非等提交时才暴露。
  - 未安装 gitleaks 时，按审查范围用 `git diff --cached` / `git diff` 人工检索 `key`、`secret`、`token`、`password`、`AKIA`、`-----BEGIN` 等特征，并在审查报告中注明「未执行工具扫描」。
  - 命中真实密钥一律 🔴；已提交过的密钥除删除外还须提醒开发者**轮换**该密钥。
...
>
> **跨模块规则**：改动涉及 ≥ 3 个模块或服务即命中。本节、§2.6、§4.3 共用此阈值，改阈值只需改这里。本节三项告警阈值同时作为 §3.12 多视角交叉复核的触发条件。
...
**检测结果驱动专项清单：**
- `SCOPE_AUTH=true` → 必跑 §3.2 安全专项清单（`SCOPE_AUTH` 覆盖鉴权、支付、隐私、权限、密钥、数据边界等安全敏感改动，不限于登录认证）
- `SCOPE_FRONTEND=true` → 必跑 §3.5 技术栈前端、§3.13 浏览器兼容性（PC 端与移动端均为强制，不得跳过），视情况跑 §3.6 复杂度
...
- §1.5 负载告警任一项触发（高负载 / 安全敏感改动超阈值 / 跨模块） → SHOULD 执行 §3.12 多视角交叉复核

在审查报告中展示检测结果：`Diff Scope（改动范围）: auth/frontend/backend/config/API/migration`

### 第 2.7 步：审查前置分析报告（统一产出）

第一步 Secret 扫描结果、第 1.5、2.5、2.6 步的产出及第 2.7 步架构定性检查，应**合并到一份统一的「审查前置分析报告」**中，置于审查报告顶部，分节呈现。避免将每个步骤作为独立输出造成信息碎片化：
...
### 🔐 Secret 扫描
- 命令：`gitleaks git --pre-commit [--staged] --redact`（或「未执行工具扫描，已人工检索」）
- 范围：暂存区 / 未暂存 diff，scanned ~N bytes
- 结果：no leaks found / 命中 N 处（仅记规则名与 文件:行号，不写密钥原文）

### 🌐 兼容性基线（SCOPE_FRONTEND=true 时必填）
- 来源：项目声明（browserslist / build.target / 需求文档）/ 默认广覆盖基线（见 `references/browser-compatibility.md`）
- PC 端：...
- 移动端：...
...
| **并发与竞态** | 「先查后写」（check-then-act）是否有锁/唯一约束/事务兜底；重复提交、重复回调、消息重投是否幂等；共享状态（全局变量、缓存、单例）是否存在并发写。 |
...
- [ ] 信任边界：跨边界进入系统的数据（用户输入、第三方回调、消息队列、LLM 输出、文件上传）是否在边界处校验和过滤（XSS、SQL 注入防护）
- [ ] 认证与鉴权：每个敏感操作前是否有权限校验
- [ ] 接口密钥/Token/密码是否未硬编码在代码中（工具兜底见第一步 Secret 扫描）
- [ ] 错误信息是否避免泄露内部细节（堆栈、数据库结构等）
- [ ] CSRF 防护：状态变更操作是否有 CSRF 保护
- [ ] SSRF：后端发起的对外请求，目标 URL 是否校验协议/主机白名单
- [ ] IDOR / 数据边界：资源访问是否校验归属（owner/租户），而非仅校验「已登录」；列表、导出、批量接口是否带租户/归属条件并限制范围
- [ ] 命令注入：拼接进 shell/子进程的输入是否经转义或参数化
- [ ] 依赖审计：是否运行 `npm audit` / `pnpm audit`；新增依赖是否核验过版本与安全
- [ ] 支付/金额：金额、价格、折扣是否由服务端计算与校验；支付回调是否校验签名（幂等见 §3.1 并发与竞态）
- [ ] 隐私数据：手机号、身份证、地址、Token 等敏感字段是否未写入日志、埋点、错误上报或前端存储；对外返回是否脱敏、最小化
...
#### 3.12 多视角交叉复核（大范围/高风险时）

> §1.5 负载告警任一项触发时 SHOULD 执行（阈值以 §1.5 为准）；其余情况可跳过。

单一视角的审查容易被第一印象锁定。触发时，由**专注审查视角的子任务**分别独立审查同一份 diff，互不共享结论：

| 视角 | 关注点 | 对应清单 |
|------|--------|---------|
| A：架构 / 类型 / 测试缺口 | 模块边界、依赖方向、类型是否准确、哪些分支没有测试覆盖 | §3.4、§3.5、§3.11 |
| B：可读性 / UX / 文档 | 读者能否看懂、交互态与异常态是否完整、多端兼容是否到位、文档是否同步 | §3.6、§3.7、§3.8、§3.10、§3.13 |

- 每个子任务只拿到 diff、需求描述和对应清单，不拿主审查的结论，避免被带偏。
- 当前 agent 不支持子任务时，由主审查按视角分轮独立执行，每轮开始前明确声明当前视角。
- 各视角发现合并进 §4.1 审查结论：重复项合并，保留**来源标注**；视角间结论冲突时列出双方理由，由开发者裁决。

#### 3.13 浏览器兼容性审查（PC 端 + 移动端，强制）

> `SCOPE_FRONTEND=true` 时 **MUST** 执行，PC 端与移动端两部分均不得跳过，局部样式改动也不例外；纯后端/文档/配置改动标注「不适用」。
>
> 默认兼容基线、高发问题清单与定级参考见 `references/browser-compatibility.md`，审查前先读。

**0. 确定兼容范围**
- [ ] 按 `references/browser-compatibility.md` §1 确定目标范围：项目声明优先，未声明时采用默认广覆盖基线，并写入前置分析报告「🌐 兼容性基线」
- [ ] 构建配置（`build.target`、legacy 插件、Babel/core-js、autoprefixer）是否真正覆盖声明的范围

**PC 端**
- [ ] 新 JS 语法是否会被转译（如正则后行断言，未转译会在旧 Safari 导致整页白屏）；新 JS API 是否有 polyfill
- [ ] 新 CSS 特性是否有 `@supports` 降级，需要前缀的属性是否由 autoprefixer 或手写前缀覆盖
- [ ] 日期解析是否避免 `new Date('YYYY-MM-DD HH:mm')` 这类 Safari 不支持的格式
- [ ] 1280/1366 宽、125%/150% 缩放下是否溢出或截断；Windows 滚动条占宽、字体回退是否影响布局
- [ ] 面向国内 PC 用户时，国产双核浏览器是否会落入 IE 兼容模式

**移动端**
- [ ] viewport meta 是否正确；`100vh` 是否改用 `dvh`/`svh` 并保留回退；刘海屏与底部安全区是否适配
- [ ] 输入框字号是否 ≥ 16px（避免 iOS 聚焦放大）；软键盘弹出时 fixed 元素和输入框是否错位或被遮挡
- [ ] 是否存在只靠 hover 触发的交互；可点击区域是否 ≥ 44×44px；弹窗是否有滚动穿透
- [ ] 320px 最小宽度与横竖屏切换下是否横向溢出、布局错乱
- [ ] 平台判断是否用特性检测而非 UA（iPadOS 默认伪装成 Mac）
- [ ] 微信等内置 WebView 中，下载、文件上传、`window.open`、自动播放等受限能力是否有替代路径

**定级**：按 `references/browser-compatibility.md` §5 执行——目标范围内白屏、报错或核心路径无法完成为 🔴。静态审查无法确认的高风险项标注「未运行验证」，并建议按该文件 §6 做运行时验证。
...
使用以下结构化格式输出结论。🔴/🟡 发现须标注**来源**：`〔代码审查〕`、`〔安全专项〕`、`〔Secret 扫描〕`、`〔兼容性〕`、`〔交叉复核-A〕`、`〔交叉复核-B〕`，便于追溯每条发现由哪项检查产生。
...
- 文档内容至少包含：修改文件列表与要点、场景矩阵、审查结论（通过/不通过及原因，发现带来源标注）、专项清单检查结果、Secret 扫描结果、**兼容性基线与 §3.13 结论（前端改动时）**、**多视角交叉复核结论（如执行过）**、**Self-Test 题目与结论（如执行过）**。
- 若由上层工作流调用并指定了输出路径（如 ai-coding-workflow Phase 7 的 `specs/<feature-id>/review-findings.md`），按调用方指定的路径写入，内容要求不变。
...
- **审查顺序**：先架构定性（§2.7）再逐行专项（§3.2-§3.10），避免在错误架构上精挑细选；安全发现按 §3.2 定级规则处理；Secret 扫描每次必跑；前端改动必跑 §3.13 浏览器兼容性审查（PC 端与移动端均强制）；大范围/高风险改动追加 §3.12 多视角交叉复核。
