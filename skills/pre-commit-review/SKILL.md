...
| B：可读性 / UX / 文档 | 读者能否看懂、交互态与异常态是否完整、文档是否同步 | §3.6、§3.7、§3.8、§3.10 |
...
使用以下结构化格式输出结论。🔴/🟡 发现须标注**来源**：`〔代码审查〕`、`〔安全专项〕`、`〔Secret 扫描〕`、`〔交叉复核-A〕`、`〔交叉复核-B〕`，便于追溯每条发现由哪项检查产生。
...
- 文档内容至少包含：修改文件列表与要点、场景矩阵、审查结论（通过/不通过及原因，发现带来源标注）、专项清单检查结果、Secret 扫描结果、**多视角交叉复核结论（如执行过）**、**Self-Test 题目与结论（如执行过）**。
...
- **审查顺序**：先架构定性（§2.7）再逐行专项（§3.2-§3.10），避免在错误架构上精挑细选；安全发现按 §3.2 定级规则处理；Secret 扫描每次必跑；大范围/高风险改动追加 §3.12 多视角交叉复核。
