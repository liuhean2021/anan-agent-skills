...
- [ ] IF 方向未定、MVP 边界未定、或影响重大：执行 `/plan-ceo-review`，将结论写入 `.specify/specs/<feature-id>/ceo-review.md`
...
- [ ] `/qa --mode=diff-aware` QA 验证通过，截图已存档
- [ ] `/speckit.checklist` 逐条验收标准确认通过
- [ ] `/ship` → CI 全绿 + ≥ 1 人 Review Approve 后合并
- [ ] CD 自动部署 staging，CI 触发 `/qa --mode=quick` 验证通过
