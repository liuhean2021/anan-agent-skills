...
- 同一 feature 内的任务并行，优先使用 `tasks.md` 中的 `[P]` 标记做拆分，再用主代理原生 subagent 能力承接
- 代码实现时，优先按文件或模块边界拆分；代码审查时，优先按视角拆分（如正确性/安全视角与可读性/UX 视角分别由不同 subagent 承担）
- 并行执行结束后，当前主代理 MUST 负责回收结论、解决冲突、补测试并统一提交
