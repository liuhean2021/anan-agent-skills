...
3. 执行 `/plan-eng-review`，深度审查图表、边界条件、失败模式
4. 将架构评审结论写入 `specs/<feature-id>/arch-review.md`
5. 将架构决策追加写入 `memory/decisions.md`
...
**必做动作**：
1. Claude 执行 `/speckit.analyze`，Codex 执行 `$speckit-analyze`，对 `spec.md`、`plan.md`、`tasks.md` 做只读一致性分析
2. 按验收标准先写失败测试
