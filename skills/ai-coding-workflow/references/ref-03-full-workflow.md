...
> 适用：Phase 0~11 完整流程，含 ceo-review 模板、spec 模板
...
**必做动作**：
1. 执行 `specify init . --ai claude`（分布式团队可加 `--branch-numbering timestamp` 避免分支编号冲突），初始化 `.specify/` 目录
...
**必做动作**：
1. 执行 `/plan-ceo-review`，寻找最优版本，压力测试需求合理性
2. 明确记录以下最小结论：目标用户、核心问题、MVP 边界、不做什么、成功指标、是否进入 `spec`
3. IF 为简版执行，THEN 仍 MUST 记录"方向已锁定的依据"与"本次不再讨论的范围"
4. 将评审结论写入 `.specify/specs/<feature-id>/ceo-review.md`

**产出物**：`.specify/specs/<feature-id>/ceo-review.md`

**退出条件**：产品方向已确认，MVP 边界清晰，且 `.specify/specs/<feature-id>/ceo-review.md` 已写入。
...
3. 将架构评审结论写入 `.specify/specs/<feature-id>/arch-review.md`
4. 将架构决策追加写入 `memory/decisions.md`

**产出物**：`.specify/specs/<feature-id>/plan.md`、`.specify/specs/<feature-id>/research.md`、`.specify/specs/<feature-id>/data-model.md`、`.specify/specs/<feature-id>/contracts/`、`.specify/specs/<feature-id>/arch-review.md`、`memory/decisions.md`（追加）
...
**进入条件**：WHEN 任务类型为新功能，Phase 3 已完成。

**必做动作**：
1. 执行 `/speckit.analyze`，检查 spec / plan 跨文档矛盾
2. 执行 `/speckit.checklist`，生成验收 checklist

**产出物**：`.specify/specs/<feature-id>/checklists/`

**退出条件**：无跨文档矛盾，checklist 已生成。

---

#### Phase 5：任务拆解

**进入条件**：WHEN 任务类型为小功能及以上，Phase 4 已完成（或 Phase 2 已完成，适用于跳过 Phase 4 的情况）。

**必做动作**：
1. 执行 `/speckit.tasks`，生成 `tasks.md`，含 `[P]` 并行标记与 TDD 标记

**产出物**：`.specify/specs/<feature-id>/tasks.md`
...
**进入条件**：WHEN 任何改动（包括 bug fix）开始实施前。

**必做动作**：
1. 按验收标准先写失败测试
2. IF 后续实现预计会调用外部 agent，THEN 先在 `tasks.md` 标注可并行项与上下文边界，避免多个 agent 重复改同一文件
3. 执行 `/commit-message` 生成提交信息，等待确认后再执行提交（格式：`wip: [<feature-id>] test cases`）

**产出物**：失败测试文件，wip commit

**退出条件**：测试已写入并提交，测试当前为失败状态（红灯）。

---

#### Phase 7：代码实施

**进入条件**：WHEN Phase 6 已完成，失败测试已就位。
...
5. 每完成一个原子任务，MUST 立即执行 `/commit-message` 生成提交信息，等待确认后再提交（格式：`wip: [<task-id>] <描述>`）；MUST NOT 直接调用 `git commit` 绕过该步骤；提交信息 MUST 使用中文（摘要与说明列表均用中文），除非用户明确要求英文
6. IF 遇到问题/踩坑，THEN MUST 将内容追加写入 `memory/issues.md`
7. IF 发现规格有误，THEN MUST 返回 Phase 2 正式修改，重走 Phase 3，MUST NOT 绕过

**产出物**：wip commits，`memory/issues.md`（如有追加）

**退出条件**：所有原子任务完成，测试全部通过（绿灯）。

---

#### Phase 8：代码审查 + 安全审查 + Secret 扫描

**进入条件**：WHEN 所有改动完成，Phase 7 退出。
...
5. 将 `/review`、`security-engineer`、OMC 外部复核中的有效发现统一汇总写入 `.specify/specs/<feature-id>/review-findings.md`，并标注来源
6. IF 存在审查发现，THEN 修复后 MUST 重新执行本 Phase

**产出物**：`.specify/specs/<feature-id>/review-findings.md`

**退出条件**：无阻断性审查发现，`.specify/specs/<feature-id>/review-findings.md` 已写入。
...
**必做动作**：
1. 执行 `/qa --mode=diff-aware`，生成 `qa-reports/`（含截图）
...
**进入条件**：WHEN Phase 9 已通过。

**必做动作**：
1. 执行 `/ship`，创建 PR + CHANGELOG
2. CI 自动执行（push 触发，无需手动）：`/review` + 安全扫描 + 测试套件 → CI gate（失败则阻断合并）
3. 人工 Code Review：至少 1 人 Approve（CODEOWNERS 强制）
4. 合并后 CD 自动部署到 staging
5. CI 自动触发 `/qa https://staging.<domain> --mode=quick`
6. 通过后人工批准生产部署
7. 上线后观察 5 分钟（监控告警）
