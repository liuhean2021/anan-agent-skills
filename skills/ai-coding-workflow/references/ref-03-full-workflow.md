...
5. 将评审结论写入 `.specify/specs/<feature-id>/ceo-review.md`
...
**必做动作**：
1. 执行 `/speckit.checklist`，生成需求质量 checklist
2. 根据 checklist 补齐规格中的完整性、清晰度、一致性问题

**产出物**：`.specify/specs/<feature-id>/checklists/`

**退出条件**：需求质量问题已闭环，checklist 已生成。
...
#### Phase 6：一致性分析 + 测试先行（TDD）

**进入条件**：WHEN Phase 5 已完成，且任何改动（包括 bug fix）开始实施前。
...
4. 执行 `/commit-message` 生成提交信息，等待确认后再执行提交（格式：`wip: [<feature-id>] test cases`）

**产出物**：分析报告、失败测试文件、wip commit
