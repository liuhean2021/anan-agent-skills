...
> 来源：ai-coding-workflow-best-practices.md §2、§10、§11
> 适用：工具选型、安装升级、命令速查
...
/speckit.specify "功能描述"      # PRD + 用户故事
/speckit.clarify                 # 澄清模糊点，最多 5 个问题（plan 前 MUST 执行）
/speckit.plan "技术栈"           # 技术方案
/speckit.analyze                 # 一致性检查（plan 后、tasks 前，MUST 执行）
/speckit.checklist               # 验收 checklist（analyze 后、tasks 前，MUST 执行）
/speckit.tasks                   # 任务拆解（analyze + checklist 完成后执行）
...
/review                          # 代码审查（生产级 bug）
/ship                            # 发布
/qa --mode=diff-aware            # 只测 git diff 影响的页面（最常用）
/qa --mode=quick                 # 30 秒冒烟测试（staging 验证用）
/qa --mode=regression            # 对比基线回归测试
/retro                           # 周复盘
```

### 10.3 agency-agents 常用角色

安装方式：将 agency-agents 仓库（msitarzewski/agency-agents）中的 `engineering/`、`testing/`、`product/` 目录下的角色文件复制到 `~/.claude/agents/`。

```bash
# 升级（在 agency-agents 仓库目录下执行）
git pull origin main
cp engineering/*.md ~/.claude/agents/
cp testing/*.md ~/.claude/agents/
cp product/*.md ~/.claude/agents/
```
...
| **agency-agents** | `git -C <agency-agents-path> pull origin main` |
