...
> | `references/ref-02-tool-stack.md` | §2 工具体系总览 + §10 各工具详细说明（spec-kit/评审-QA-发布能力/gitleaks/自检）+ §11 参考来源 |
...
> | `references/ref-06-branch-parallel.md` | §8 分支策略（主干开发、PR合并规则）+ §9 并行开发（Git Worktree + 主代理原生并行能力选型） |
> | `references/ref-07-advanced-practices.md` | §12 进阶实践（P1 测试分层/依赖安全/DORA/Hooks；P2 AI信任边界/环境策略/合规审查/成本管理） |
> | `references/ref-08-verification-gate.md` | §13 验证铁律 + 阶段顺序纪律（本技能内置横切纪律层、Iron Law、Gate Function） |
> | `references/ref-09-experience-quality.md` | §14 经验质量判定（三镜头 + 九类垃圾排除 + 写入前检查），`memory/patterns.md` 写入前必读 |

# AI Coding 工作流

**核心原则**：规格驱动 → 测试先行 → 最小改动 → 审查验证 → 持续复盘  
**纪律层**：Phase 按文档顺序推进；任何完成宣称 **必须附 fresh 验证证据**（详见 `references/ref-08-verification-gate.md`）；该纪律内置于本技能，任何有能力的 agent 原生遵守，不依赖外部插件  
...
**验证纪律不可降级**：Iron Law、Gate Function（见 `ref-08-verification-gate.md`）是本技能自带的横切规则，任何有能力的 agent 原生遵守，不因工具可用性而降级。
...
每个阶段可按需接入不同能力：阶段顺序、验证铁律由本技能自带的横切纪律层保障（见 `ref-08`），`spec-kit` 负责规格/方案/任务/实现骨架，方向评审、架构评审、审查、QA、发布、复盘与多模型并行/交叉复核均由 agent 自身或专门子任务承载（不依赖特定外部工具），Context7 MCP 负责核对上游官方文档，`gitleaks` 负责 Secret 扫描，`memory` 负责沉淀决策与踩坑记录。
...
执行一致性分析 → 先写失败测试 → 执行实现（必要时用主代理原生 subagent 能力并行）→ 绿灯。具体页面实现与组件开发在 Phase 6 执行，不在需求阶段提前落代码。IF 涉及前端实施，THEN Agent MUST 将 `DESIGN.md` 中的 design tokens 映射为 CSS 变量或 Tailwind 配置，MUST NOT 在组件中硬编码颜色/字体/间距值。
...
**A7 复盘**  
结构化复盘 → 有价值经验写入 `memory/patterns.md`（写入前先按 `references/ref-09-experience-quality.md` 做质量判定：三镜头 + 九类垃圾排除 + 去重合并）。复盘输出末尾 MUST 执行「收尾反思两问」并如实作答：**① 眼下最没把握的是什么？② 我最大的遗漏是什么？我没意识到什么？**（前者源自 Sam Altman，后者源自 Claude；暴露本次迭代的置信度边界与盲点，不否定复盘结论）。
...
Phase 6：实施（spec-kit implement；必要时用 subagent 并行 → 绿灯 → 原子提交）
...
④ 按 ref-08 Gate Function 确认测试通过（附 fresh 命令输出）
...
进入场景 E 或用户明确要求时，代理 SHOULD 按 `§ 10.5` 检查能力工具版本。默认不在每次会话开始时自动升级工具。

---

## 场景 F：多功能并行开发

> Git Worktree 命令 + 多代理并行选型 → `references/ref-06-branch-parallel.md`
...
| 场景 | 方案 |
|------|------|
| 同一任务内多模型分工实现 | 主代理原生 subagent / 并行任务能力，按文件或模块边界拆分 |
